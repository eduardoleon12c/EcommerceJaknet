...
export class LandingProductComponent implements OnInit {
  ...
  getDiscount(){
    ...
    if(this.SALE_FLASH){
      if(this.SALE_FLASH.type_discount == 1){
        return this.SALE_FLASH.discount*this.product_selected.price_pesos*0.01
      }else{
  ...
  addCart(product:any) {
    ...
    let data = {
      ...
      subtotal: this.product_selected.price_pesos - this.getDiscount(),//*$("#qty-cart").val()
      total: (this.product_selected.price_pesos - this.getDiscount())*$("#qty-cart").val(),
    }
